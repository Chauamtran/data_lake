...
if len(sys.argv) == 1 or len(sys.argv) == 2:
    ...
    mongo_host = ast.literal_eval(database_config.readConfig(sys.argv[1], 'mongo_host'))
    mongo_port = ast.literal_eval(database_config.readConfig(sys.argv[1], 'mongo_port'))
    mongo_username = ast.literal_eval(database_config.readConfig(sys.argv[1], 'mongo_username'))
    mongo_password = ast.literal_eval(database_config.readConfig(sys.argv[1], 'mongo_password'))
    mongo_replica_set = ast.literal_eval(database_config.readConfig(sys.argv[1], 'mongo_replica_set'))
    kafka_broker_list = ast.literal_eval(database_config.readConfig(sys.argv[1], 'kafka_broker_list'))
    kafka_example_topic_mongo = ast.literal_eval(database_config.readConfig(sys.argv[1], 'kafka_example_topic_mongo'))
...
def processData(last_id=None):
    ...
    try:
        ...
        checkAndCreateTopics(topic_name=kafka_example_topic_mongo)
        while len(data_list) > 0:
            logger.info(msg='Data Size: {}\n'.format(len(data_list)))
            produceDataToKafka(data=data_list, topic=kafka_example_topic_mongo)
