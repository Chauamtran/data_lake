...
function upServices {
  ...
  docker run -d --net host \
              ...
              -e KAFKA_ADVERTISED_LISTENERS=PLAINTEXT://kafka-3:39092 \
...
function checkServices {
  ...
  docker run --net=host \
             --rm confluentinc/cp-kafka:"${VERSION}" bash -c "echo stat | nc kafka-3 39092 | grep Mode"
}
...
if [[ $1 = 'start' ]]; then
  ...
  echo "Usage: kafka_3.sh start|stop|up|down. Remember to start zookeeper before starting kafka"
