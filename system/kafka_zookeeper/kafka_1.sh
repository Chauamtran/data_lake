...
function checkServices {
  ...
  docker run --net=host \
             --rm confluentinc/cp-kafka:"${VERSION}" bash -c "echo stat | nc kafka-1 19092 | grep Mode"
}
...
if [[ $1 = 'start' ]]; then
  ...
  echo "Usage: kafka_1.sh start|stop|up|down. Remember to start zookeeper before starting kafka"
