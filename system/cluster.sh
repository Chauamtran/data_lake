...
if [[ $1 = "start" ]]; then
  ...
  docker run -d --restart always --net hadoopnet --ip 172.18.1.4 --hostname psqlhms --name psqlhms -it postgresql-hms:"${version}"
  sleep 5

  # 3 nodes
  echo ">> Starting nodes master and worker nodes ..."
  docker run -d --restart always --net hadoopnet --ip 172.18.1.1 --hostname nodemaster --add-host node2:172.18.1.2 --add-host node3:172.18.1.3 --name nodemaster -it hive:"${version}"
  docker run -d --restart always --net hadoopnet --ip 172.18.1.2 --hostname node2 --add-host nodemaster:172.18.1.1 --add-host node3:172.18.1.3 --name node2 -it spark:"${version}"
  docker run -d --restart always --net hadoopnet --ip 172.18.1.3 --hostname node3 --add-host nodemaster:172.18.1.1 --add-host node2:172.18.1.2 --name node3 -it spark:"${version}"
...
echo "Usage: cluster.sh start|stop|uninstall version"
echo "                 start  - start existing containers"
echo "                 stop   - stop running processes"
echo "                 uninstall - remove all docker images"
echo "                 version - version of docker images"
